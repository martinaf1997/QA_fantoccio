...
| **Bulk multi-campo** (export TPS batch) | *nessuna* (di solito) | Commissioning **oppure calcolo TPS** | Matrice CSV con **tutti i field size in un unico file**: colonne = field size [mm], righe = profondità (PDD) o distanza off-axis (profili). Per i profili il file contiene un blocco per ciascuna profondità (`Curves at depth [mm]: ...`). Riconosciuto **dal contenuto** (metadati `column legend:`/`row legend:`), non serve estensione. Esistono due varianti, distinte automaticamente dal campo `data:` del file: **misurato** (`OPD`/`OPP`) e **calcolato dal TPS** (`OPD_calculated`/`OPP_calculated`, es. algoritmo Acuros/AAA) — quest'ultima viene etichettata `TPS-calc (algoritmo)` nelle curve, utile per confrontare le misure anche con il calcolo del TPS oltre che con il commissioning misurato. |
